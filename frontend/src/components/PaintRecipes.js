...
export default function PaintRecipes() {
  ...
  const load = useCallback(async () => {
    ...
    } catch { toast.error(t('paint_load_recipes_err')); } finally { setLoading(false); }
  ...
  const save = async () => {
    if (!form.color_name.trim()) { toast.error(t('paint_color_name_req')); return; }
    const body = {
      ...form,
    ...
    try {
      ...
      toast.success(isNew ? t('paint_recipe_created') : t('paint_recipe_updated'));
      setEditing(null); load();
    } catch (e) { toast.error(e.message); }
  };
  const del = async (r) => {
    if (!window.confirm(t('paint_recipe_delete_confirm', { name: r.color_name }))) return;
  ...
  const printLabel = (r) => {
    ...
    if (!w) return;
    w.document.write(`<html><head><title>${t('paint_label_title', { name: r.color_name })}</title><style>
      ...
      <div class="big">${t('paint_ingredients')}</div><table>${ings}</table>
      <div class="big">${t('paint_total_volume_is', { vol: r.total_volume || 0, unit: r.unit || 'g' })}</div>
      ${r.notes ? `<p class="p">${r.notes}</p>` : ''}
      <p class="p" style="margin-top:12px">${t('paint_printed_at', { date: new Date().toLocaleString('es-MX') })}</p></div>
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input value={q} onChange={e => setQ(e.target.value)} placeholder={t('paint_search_recipes_placeholder')}
        ...
        <button onClick={openNew} className="px-4 py-2.5 bg-primary text-black rounded-xl font-black text-xs uppercase tracking-widest flex items-center gap-2"><Plus size={16} /> {t('paint_new_recipe')}</button>
      </div>

      {loading ? (
        <div className="flex justify-center py-16"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div>
      ) : recipes.length === 0 ? (
        <div className="text-center text-muted-foreground py-16 text-sm">{t('paint_no_recipes_yet')}</div>
      ) : (
          ...
          {recipes.map(r => (
            <div key={r.recipe_id} className="bg-card/60 border border-border rounded-2xl p-4">
              ...
              <div className="mt-3 space-y-1">
                ...
                {(r.ingredients || []).length > 4 && <div className="text-[11px] text-muted-foreground">{t('paint_n_more', { n: r.ingredients.length - 4 })}</div>}
              </div>
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-border/40">
                <span className="text-xs font-bold">{r.total_volume || 0} {r.unit || 'g'}{r.cost ? ` · $${r.cost}` : ''}</span>
                <div className="flex items-center gap-1">
                  <button onClick={() => printLabel(r)} title={t('paint_print_label')} className="p-1.5 rounded hover:bg-secondary/40"><Printer size={15} /></button>
                  <button onClick={() => openEdit(r)} title={t('edit')} className="p-1.5 rounded hover:bg-secondary/40"><Pencil size={15} /></button>
                  <button onClick={() => del(r)} title={t('delete')} className="p-1.5 rounded hover:bg-secondary/40 text-red-400"><Trash2 size={15} /></button>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setEditing(null)}>
          <div className="bg-card border border-border rounded-2xl w-full max-w-lg max-h-[90vh] overflow-auto p-5" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-black uppercase tracking-widest text-sm">{editing.recipe_id ? t('paint_edit_recipe') : t('paint_new_recipe')}</h3>
              <button onClick={() => setEditing(null)} className="p-1.5 rounded hover:bg-secondary/40"><X size={18} /></button>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2"><label className="text-[10px] uppercase font-black text-muted-foreground">{t('paint_color_req_label')}</label>
            ...
            <div className="mt-4">
              <div className="flex items-center justify-between mb-1"><label className="text-[10px] uppercase font-black text-muted-foreground">{t('paint_ingredients')}</label>
                <button onClick={addIng} className="text-[11px] text-primary font-bold flex items-center gap-1"><Plus size={12} /> {t('add')}</button></div>
              <div className="space-y-2">
                {form.ingredients.map((row, i) => (
                  <div key={i} className="flex gap-2">
                    <input value={row.name} onChange={e => setIng(i, 'name', e.target.value)} placeholder={t('paint_component')} className="flex-1 min-w-0 bg-background border border-border rounded-lg px-2 py-1.5 text-sm" />
                    <input value={row.qty} onChange={e => setIng(i, 'qty', e.target.value)} placeholder={t('paint_qty_short')} type="number" className="w-20 bg-background border border-border rounded-lg px-2 py-1.5 text-sm" />
            ...
            <div className="mt-4 bg-secondary/30 rounded-xl p-3">
              <div className="flex items-center gap-1.5 text-[11px] font-black uppercase text-muted-foreground mb-2"><Calculator size={13} /> {t('paint_volume_estimator')}</div>
              <div className="grid grid-cols-3 gap-2">
                <input value={est.area} onChange={e => setEst(p => ({ ...p, area: e.target.value }))} placeholder={t('paint_area_in2')} type="number" className="bg-background border border-border rounded-lg px-2 py-1.5 text-sm" />
                <input value={est.prints} onChange={e => setEst(p => ({ ...p, prints: e.target.value }))} placeholder={t('paint_n_pieces_placeholder')} type="number" className="bg-background border border-border rounded-lg px-2 py-1.5 text-sm" />
              ...
              <div className="flex items-center justify-between mt-2 text-sm"><span className="text-muted-foreground">≈ <b className="text-foreground">{estTotal} g</b> {t('paint_of_ink')}</span>
                <button onClick={() => setForm(p => ({ ...p, total_volume: String(estTotal), unit: 'g' }))} className="text-[11px] text-primary font-bold">{t('paint_use_as_volume')}</button></div>
            </div>

            <div className="grid grid-cols-3 gap-3 mt-4">
              <div><label className="text-[10px] uppercase font-black text-muted-foreground">{t('paint_total_volume')}</label>
              ...
              <div><label className="text-[10px] uppercase font-black text-muted-foreground">{t('paint_cost')}</label>
            ...
            <div className="mt-3"><label className="text-[10px] uppercase font-black text-muted-foreground">{t('paint_notes')}</label>

                  ...
                  <table className="text-sm border-collapse" style={{ minWidth: '100%' }}>
                    <thead className="sticky top-0 z-30">
                      <tr className={isDark ? 'bg-zinc-900 border-b-2 border-border' : 'bg-gray-200 border-b-2 border-gray-300'}>
                        <th className={`w-11 py-3.5 px-2 sticky left-0 z-30 ${isDark ? 'bg-zinc-900' : 'bg-gray-200'}`}><input type="checkbox" checked={selectedOrders.length === orders.length && orders.length > 0} onChange={(e) => e.target.checked ? handleSelectAll() : handleDeselectAll()} className="w-4 h-4 rounded" data-testid="select-all-checkbox" /></th>
                        <th className={`w-10 py-3.5 px-1 sticky left-[48px] z-30 ${isDark ? 'bg-zinc-900' : 'bg-gray-200'}`}></th>
                        ...
                        {visibleColumns.map(col => {
                          ...
                          return (
                            <th key={col.key} className={`py-3.5 px-3 text-left font-barlow uppercase text-base font-extrabold tracking-wider ${isDark ? 'text-zinc-200' : 'text-gray-800'} ${draggedCol === col.key ? 'opacity-50' : ''} ${isOrderNum ? `sticky left-[88px] z-30 ${isDark ? 'bg-zinc-900' : 'bg-gray-200'}` : ''}`} style={{ minWidth: columnWidths[col.key] || col.width }} data-testid={`column-header-${col.key}`} draggable onDragStart={() => handleColumnDragStart(col.key)} onDragOver={(e) => handleColumnDragOver(e, col.key)} onDragEnd={handleColumnDragEnd}>
                        ...
                        const renderOrderRow = (order) => {
                          ...
                          return (
                            <tr key={order.order_id} className={`border-b transition-colors ${isDark ? 'border-border/50 hover:bg-zinc-800/50' : 'border-gray-200 hover:bg-gray-100'} ${selectedOrders.includes(order.order_id) ? (isDark ? 'bg-primary/10' : 'bg-blue-50') : ''} ${isSearchMatch ? 'ring-2 ring-primary ring-inset bg-primary/10' : ''}`} data-testid={`order-row-${order.order_id}`}>
                              ...
                              {visibleColumns.map(col => (
                                <td key={col.key} className={`py-2 px-3 ${col.key === 'order_number' ? `sticky left-[88px] z-10 ${isSearchMatch ? 'bg-primary/10' : selectedOrders.includes(order.order_id) ? (isDark ? 'bg-primary/10' : 'bg-blue-50') : (isDark ? 'bg-background' : 'bg-white')}` : ''}`} style={{ minWidth: columnWidths[col.key] || col.width }}>
                    ...
                    <tfoot className={`sticky bottom-0 z-20 ${isDark ? 'bg-zinc-900 border-t-2 border-border' : 'bg-gray-200 border-t-2 border-gray-300'}`}>
                      <tr className={isDark ? 'bg-zinc-900/95 border-t-2 border-border' : 'bg-gray-200/95 border-t-2 border-gray-300'}>
                        <td className={`py-2 px-2 sticky left-0 z-20 ${isDark ? 'bg-zinc-900' : 'bg-gray-200'}`}></td>
                        <td className={`py-2 px-1 sticky left-[48px] z-20 ${isDark ? 'bg-zinc-900' : 'bg-gray-200'}`}></td>
                        ...
                        {visibleColumns.map(col => {
                          ...
                          return <td key={col.key} className={`py-2 px-3 ${isOrderCol ? `sticky left-[88px] z-20 ${isDark ? 'bg-zinc-900' : 'bg-gray-200'}` : ''}`} style={{ minWidth: columnWidths[col.key] || col.width }}>{total !== null ? <span className="font-barlow font-bold text-sm text-primary" data-testid={`footer-total-${col.key}`}>{Number.isInteger(total) ? total.toLocaleString() : total.toFixed(2)}</span> : isOrderCol ? <span className="font-barlow font-bold text-sm text-primary">{t('total')}</span> : null}</td>;
                        })}

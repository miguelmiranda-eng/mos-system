...
const RELEASE_ROLES = ['supersu', 'inspector_qc', 'qc'];
...
export default function QCDashboard() {
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      const res = await fetch(`${API}/orders/${statusEditOrder.order_id}`, {
        method: 'PUT',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className={cn("h-screen overflow-y-auto flex flex-col pb-10", base)}>
      ...
      {statusEditOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => !savingStatus && setStatusEditOrder(null)}>
          <div className={cn("w-full max-w-md rounded-2xl border shadow-2xl", isDark ? "bg-slate-900 border-white/10" : "bg-white border-slate-200")} onClick={e => e.stopPropagation()}>

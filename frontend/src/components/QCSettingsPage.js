  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      <header className="sticky top-0 z-40 bg-card border-b border-border px-4 md:px-8 h-16 flex items-center gap-3 flex-shrink-0">
        <button onClick={() => navigate("/home")} className="p-2 rounded-lg hover:bg-secondary text-muted-foreground" title={t("qc_back_home")}>
          <ArrowLeft className="w-5 h-5" />
        </button>
        <ShieldCheck className="w-5 h-5 text-primary" />
        <div className="font-black uppercase tracking-tight">{t("qc_settings_title")}</div>
      </header>

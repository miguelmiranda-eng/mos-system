...
import { History, Filter, X, Loader2, Undo2, Search } from "lucide-react";
...
export const ActivityLogModal = ({ isOpen, onClose, onUndoSuccess, t }) => {
  ...
  useEffect(() => { if (isOpen) fetchLogs(); }, [isOpen, actionFilter, debouncedSearch]);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[95vw] md:max-w-5xl max-h-[85vh] bg-card border-border overflow-hidden flex flex-col" data-testid="activity-log-modal">
        <DialogHeader>
          <DialogTitle className="font-roboto font-black text-xl uppercase tracking-widest flex items-center gap-2 text-glow-primary">
            <History className="w-5 h-5" /> {t('activity_title')} ({total} {t('activity_records')})
          </DialogTitle>
        </DialogHeader>
        <div className="flex flex-col md:flex-row items-start md:items-center gap-4 pb-4 border-b border-border px-1">
          <div className="flex items-center gap-2">
            <Filter className="w-4 h-4 text-muted-foreground" />
            <Select value={actionFilter} onValueChange={setActionFilter}>
              <SelectTrigger className="w-48 h-9 text-xs bg-secondary border-border" data-testid="activity-filter"><SelectValue placeholder={t('filter_action_placeholder')} /></SelectTrigger>
           ...
           logs.length === 0 ? <p className="text-center text-muted-foreground py-8">No hay registros</p> : (
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-card z-10"><tr className="border-b border-border">
                ...
                <th className="text-left py-2 px-3 font-roboto uppercase text-[10px] tracking-[0.2em] text-muted-foreground w-32">{t('user')}</th>
                <th className="text-left py-2 px-3 font-roboto uppercase text-[10px] tracking-[0.2em] text-muted-foreground w-36">{t('action_label')}</th>
                <th className="text-left py-2 px-3 font-roboto uppercase text-[10px] tracking-[0.2em] text-muted-foreground">{t('details')}</th>
                <th className="text-right py-2 px-3 font-roboto uppercase text-[10px] tracking-[0.2em] text-muted-foreground w-24">{t('undo')}</th>
              </tr></thead>
              <tbody>{logs.map(log => (
